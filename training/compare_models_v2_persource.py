...
def fit(t):
    ...
    for T in np.arange(0.5,1.31,0.05):
        for thr in np.arange(0.05,0.8,0.01):
            ...
            if min(se)>=0.92 and (best is None or np.mean(spc)>best[0]): best=(np.mean(spc),T,thr,np.mean(se))
    return best
...
for t,n in (('old','old'),('new','v1 (FGADR-only)'),('v2','v2 (mixed)')):
    f=fit(t); print(f'{n}: PER-SOURCE recalibrated T={f[1]:.2f} thr={f[2]:.2f} (cal sens={f[3]:.3f} spec={f[0]:.3f})'); cfg[f'{n} @ recal']=(t,f[1],f[2])
